...
| `src/data/` | Les données figées : catégories, 101 schémas de sous-catégories, découpage district → région → ville → commune, et `pays.ts` — les 117 pays hors Côte d'Ivoire, rangés comme des « villes » de la région `autres-pays` (la ville réelle s'écrit dans `commune`). Mêmes listes dans `flutter_app/lib/data/` ; `npm run banc:coherence` les compare. |
