...
| Date du build | ✅ **CONSTRUITE le 13/09/2026**, par le Patron sur son Mac. **Du premier coup, en 63,9 s** — aucun des trois échecs de la veille ne s'est reproduit : `android/key.properties` a survécu au passage de l'outil, comme prévu. Marche à suivre : `store/BUILD-v1.26.md`. |
| Poids de l'AAB | **69,3 Mo — exactement le même que la v1.25.** La prédiction tenait : l'avis n'ajoute aucun greffon natif, donc aucun poids. ⚠️ **Ce chiffre reste celui du BUNDLE, pas du téléchargement.** Le vrai chiffre — celui que paie un utilisateur en forfait de données — s'affiche dans la Play Console après dépôt, et **n'a toujours été relevé pour aucune version du projet.** |
| **iPhone** | ✅ **Installée le 13/09/2026** — `flutter run --release`, build Xcode en 35,8 s, installation en 6,1 s, équipe `JT6C59KB84`. Reparti pour 7 jours. |
...
| Champ | Valeur |
|---|---|
| **Commit** | ⚠️ **RECTIFIÉ UNE TROISIÈME FOIS LE 12/09/2026 : c'est `0a8440f`**, « Les boutons d'action redeviennent orange » (10/09), et non plus `7b931b4`. Ce commit corrige **huit boutons d'action qui sortaient verts** depuis le 30/08 — la constante `ChapColors.orange` était passée au vert et les boutons qui la recopiaient n'avaient pas suivi — plus une cible tactile de 40 px. Un Monteur qui aurait construit `7b931b4` aurait livré aux testeurs une application aux boutons de la mauvaise couleur, juste au moment où Google nous reproche leur manque d'engagement. **Trois rectifications sur le même champ en cinq jours : la fiche de build ne compare donc plus à un numéro figé** — elle fait comparer `HEAD` à `origin/`, ce qui ne vieillit pas. Ancienne valeur, gardée pour l'histoire : `7b931b4` — **le dernier commit qui touche `flutter_app/` au 08/09/2026** (ce que le Monteur doit construire ; un commit plus récent qui ne touche que `store/` ou le site ne change rien à l'AAB), pas celui de la montée de version. **Rectifié une seconde fois le 08/09/2026** : le champ portait `19bbd6e`, arrêté au 07/09 au soir ; le chantier Firebase du 08 (`5accb8d`, l'application demande son jeton à FCM ; `7b931b4`, `.metadata` cesse d'être suivi) est venu après. Un Monteur qui aurait construit `19bbd6e` aurait livré une application **sans les notifications** — et sans rien pour le lui dire, puisque l'absence de Firebase est silencieuse par construction. Avant ces deux-là : `4b7a22c` n'a porté que `1.25.0+26` dans `pubspec.yaml` ; **vingt et un commits `flutter_app/` sont venus après lui** (vidéo d'une minute, affiche WhatsApp, « Faire une offre », annonce écrite depuis la photo, recherche ivoirienne, offres d'emploi, comptes hors CI, stock des comptes Pro, liens de notification, don et biométrie, vidéo qui ne se lance plus seule), tous racontés dans le corps de cette fiche. Première rectification le 07/09/2026, sur signalement du bureau Livraison. `4b7a22c` n'a porté que `1.25.0+26` dans `pubspec.yaml` ; **vingt et un commits `flutter_app/` sont venus après lui** (vidéo d'une minute, affiche WhatsApp, « Faire une offre », annonce écrite depuis la photo, recherche ivoirienne, offres d'emploi, comptes hors CI, stock des comptes Pro, liens de notification, don et biométrie, vidéo qui ne se lance plus seule), tous racontés dans le corps de cette fiche. Le champ a été rectifié le 07/09/2026 sur signalement du bureau Livraison — il indiquait encore `4b7a22c`, et un Monteur qui l'aurait cru aurait construit une application vieille de seize commits. `pubspec.yaml` reste à `1.25.0+26` : la valeur est bonne, jamais construite ni téléversée, **on n'y touche pas**. |
| Date du build | ✅ **CONSTRUITE le 12/09/2026**, par le Patron sur son Mac. Trois essais : le premier a tout compilé et buté sur `validateSigningRelease` (`key.properties` jamais rempli — le `cp` du modèle avait été rejoué et l'avait écrasé) ; le deuxième a atteint `signReleaseBundle` et buté sur « keystore password was incorrect » ; le troisième est passé **en 5,6 s**. Cause des deux échecs : le keystore est un **PKCS12**, format qui exige `keyPassword` **identique** à `storePassword` — voir l'encadré « Le keystore de signature » plus haut. |
| Poids de l'AAB | **69,3 Mo** — contre **50,3 Mo** pour la v1.20 du 15/08. **+19 Mo, soit +38 %.** L'écart est attendu (Firebase, `local_auth`, `video_player`, la bibliothèque `image`) mais **il n'a pas été mesuré poste par poste**, et personne ne devrait le croire justifié sur cette seule foi. ⚠️ **Ce chiffre n'est PAS ce que télécharge un utilisateur** : un AAB contient toutes les architectures, et le Play Store en sert une seule. **Le vrai chiffre est celui que la Play Console affiche après le dépôt** (« taille du téléchargement ») — à relever et à écrire ici, il n'a jamais été noté pour aucune version. C'est lui qui compte en Côte d'Ivoire, où le forfait de données se compte. |
| ⚠️ **minSdk RÉEL : 24, pas 23** | Relevé **dans la Play Console le 12/09/2026**, après le dépôt : « **API 24 ou version ultérieure** ». Or `preparer_plateformes.dart` écrit `minSdk 23` et l'annonce à chaque exécution, et toutes nos fiches disent « Android 6.0 minimum ». **C'est faux.** Une bibliothèque de la chaîne exige 24 et le fusionneur de manifestes retient le maximum, en silence. Conséquence réelle : **Android 6.0 (API 23) est exclu en plus d'Android 5.1** — une bande d'appareils de plus que ce qu'on avait annoncé, sur un marché où les téléphones sont vieux. **La valeur que nous déclarons n'est pas celle qui s'applique, et seule la Play Console nous l'a appris.** À trancher (voir la ligne suivante), pas à corriger en douce. |
| minSdk déclaré **23** · targetSdk **36** | signature `CN=Chap.ci` — SHA-1 `0E:C0:…:FE:33`. **Le plancher est monté de 22 (Android 5.1) à 23 (Android 6.0) le 08/09/2026** : `firebase_core` 4.14.0 le pose lui-même dans `android/local-config.gradle`, ce n'est pas un choix qu'on peut contourner en gardant Firebase. À vérifier en Play Console → Statistiques → répartition par version d'Android **avant** de construire ; si Android 5.x pèse encore, il faut rendre Firebase optionnel (une demi-journée) plutôt que de perdre ces appareils. |
| État Play | ✅ **EN LIGNE SUR LE CANAL DE TEST FERMÉ — CONFIRMÉ PAR LE PATRON DANS LA CONSOLE**, capture du 13/09/2026 à 00 h 0x. La ligne dit : `26 (1.25.0)` · canal **Tests fermés – Test fermé Chap.ci** · **« Disponible pour les testeurs sur Google Play »** · **Déploiement complet** · dernière mise à jour **12 sept. 2026 23:36** · **177 pays sur 177**. La dernière porte — *Envoyer les modifications pour examen* — a donc bien été franchie, contrairement aux v1.1 et v1.16 restées en brouillon. **Première mise à jour reçue par les douze testeurs depuis la v1.20 du 15/08.** ⚠️ **Le chiffre à surveiller est dans la même ligne : « Nombre d'installations (par utilisateur actif) : 0,00 % »** — normal vingt minutes après le déploiement, mais **c'est exactement la mesure sur laquelle Google a refusé l'accès en production**. À relever chaque jour jusqu'au 26/09 ; s'il reste à 0, la seconde demande sera refusée comme la première. *(Pour comparaison, la ligne `1.3` du test interne, déployée le 28/07, affiche 33,33 %.)* Ancienne valeur de ce champ, gardée pour l'histoire : « NON VÉRIFIÉ — non construite, donc non téléversée ». La marche à suivre Android est **`store/BUILD-v1.25.md`**, écrite le 08/09/2026 pour CE build : le code 26, le commit `7b931b4`, les trente-sept chantiers depuis la v1.20, l'encadré Firebase (le `google-services.json` à poser dans `flutter_app/tool/secrets/` avant de construire, sans quoi l'application se construit très bien et ne reçoit **aucune** notification, en silence), et la voie rapide pour voir l'application sur son propre téléphone sans passer par le magasin. Ne suivez plus `BUILD-v1.24.md` : sa version n'a jamais été construite et elle annonce le code 25. |
...
« À propos » (`lib/widgets/reseaux_pro.dart`). Les icônes sont dessinées au
...
| État Play | **Téléversée et envoyée à l'examen le 26/08/2026** — confirmé par le Patron (capture « Vue d'ensemble de la publication » : *Tests fermés — 22 (1.21.0), modifications en cours d'examen*, publication gérée désactivée → diffusion automatique aux testeurs au verdict). Échéance `targetSdk 36` du 30/08 : **tenue**. Le décompte des 14 jours (12ᵉ testeur inscrit le 26/08) court indépendamment. ✅ **VERDICT ENFIN CONNU — LE 13/09/2026, ET PAR UNE VOIE INATTENDUE.** La page *Commentaires sur les tests* de la Play Console porte un retour d'un testeur daté du **12/09/2026 à 11 h 57**, et la fiche technique de ce retour dit : *« Code de la version de l'application : **22** · Nom de la version : **1.21.0** »*. **La v1.21 a donc bien passé l'examen et atteint les testeurs** — ce champ est resté « NON VÉRIFIÉ » dix-huit jours faute que quiconque ouvre cette page. |
