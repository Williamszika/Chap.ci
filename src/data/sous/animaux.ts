...
const SOUS = ['Volaille', 'Bétail & Élevage', 'Chiens & Chats', 'Oiseaux, Poissons & Reptiles',
...
function usageAnimal(o: string[]): ChampCourt {
...
function sante(quoi: string): ChampCourt[] {
  return [
...
const SCHEMAS: Record<string, SchemaSous> = {
  ...
  'Volaille': {
    ...
    titre: function (S: Vals) { return [S.especeV, S.ageV, S.nombreV].filter(Boolean).join(' · ') },
    champs: ([
      ...
      { k:'poidsV', l:'Poids moyen', t: 'text' as const, ph:'Ex : 1,8 kg par sujet',
    ...
    ] as ChampCourt[]).concat(sante('la bête')).concat([
      { k:'ponte', l:'Production d’œufs',
        when:function (S: Vals) { return /pondeuse|Caille|Canard|Pintade/.test(S.especeV || '') },
  ...
  'Bétail & Élevage': {
    ...
    titre: function (S: Vals) { return [S.especeB, S.sexeB, S.ageB].filter(Boolean).join(' · ') },
    champs: ([
      ...
      { k:'poidsB', l:'Poids approximatif', t: 'text' as const, req:true, ph:'Ex : 35 kg',
      ...
      { k:'raceB', l:'Race', t: 'text' as const, ph:'Ex : Djallonké, Sahélien, Balami' },
      { k:'gestation', l:'Gestation',
        when:function (S: Vals) { return S.sexeB === 'Femelle' },
  ...
  'Chiens & Chats': {
    ...
    titre: function (S: Vals) { return [S.especeC, S.raceC, S.ageC].filter(Boolean).join(' · ') },
    champs: ([
      { k:'especeC', l:'Animal', req:true, o:['Chien','Chiot','Chat','Chaton'] },
      { k:'raceC', l:'Race', t: 'text' as const, req:true, ph:'Ex : Berger allemand, Rottweiler, croisé, chat de gouttière',
      ...
      { k:'sevrage', l:'Sevrage', req:true,
        when:function (S: Vals) { return /Chiot|Chaton/.test(S.especeC || '') || /Moins de 8|8 semaines/.test(S.ageC || '') },
    ...
    ] as ChampCourt[]).concat(sante('l’animal'))
  },
  ...
  'Oiseaux, Poissons & Reptiles': {
    ...
    titre: function (S: Vals) { return [S.especeO, S.ageO].filter(Boolean).join(' · ') },
    champs: ([
  ...
  'Aliments pour animaux': {
    ...
    titre: function (S: Vals) { return [S.typeAl, S.pourQui, S.condAl].filter(Boolean).join(' · ') },
    champs: [
      ...
      { k:'stade', l:'Stade',
        when:function (S: Vals) { return /Volaille|Porc|Bétail/.test(S.pourQui || '') },
      ...
      { k:'marqueAl', l:'Marque ou fabricant', t: 'text' as const, ph:'Ex : Ivograin, provende maison' },
      { k:'grosAl', l:'Vente en gros', t: 'toggle' as const }
    ]
  },
  ...
  'Accessoires & Matériel': {
    ...
    titre: function (S: Vals) { return [S.typeMat, S.pourQuiM].filter(Boolean).join(' · ') },
    champs: [
      ...
      { k:'capaciteMat', l:'Capacité',
        when:function (S: Vals) { return /Cage|Poulailler|Couveuse|Aquarium|Mangeoire|Abreuvoir/.test(S.typeMat || '') },
      ...
      { k:'dimsMat', l:'Dimensions', t: 'text' as const, ph:'Ex : 120 × 60 × 90 cm' },
      { k:'grosMat', l:'Vente en gros', t: 'toggle' as const }
    ]
  }
}

export const ANIMAUX: DonneesCat = {
  sous: SOUS,
