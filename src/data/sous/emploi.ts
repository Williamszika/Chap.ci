...
function fraisCandidat(quoi: string): ChampCourt {
...
function contactCandidature(): ChampCourt {
...
const EXPERIENCE = ['Débutant accepté','Moins d’un an','1 à 2 ans','3 à 5 ans','5 à 10 ans','Plus de 10 ans']
const NIVEAU = ['Sans diplôme exigé','CEPE','BEPC','BAC','BAC+2 (BTS, DUT)','BAC+3 (Licence)','BAC+5 (Master, ingénieur)','Doctorat','Formation professionnelle / CQP']
...
const SCHEMAS: Record<string, SchemaSous> = {
  ...
  'Offres d’emploi': {
    ...
    champs: [
      ...
      { k:'entreprise', l:'Nom de l’entreprise', t: 'text' as const, req:true, ph:'Ex : Boulangerie Le Croissant, SARL Kouassi & Fils',
      ...
      { k:'dureeCdd', l:'Durée du contrat',
        when:function (S: Vals) { return /CDD|Intérim|Journalier|Apprentissage/.test(S.contrat || '') },
  ...
  'Demandes d’emploi': {
    ...
    champs: [
      ...
      { k:'compétences', l:'Compétences', t: 'text' as const, ph:'Ex : Excel, caisse, soudure à l’arc, couture',
  ...
  'Stages': {
    ...
    titre: function (S: Vals) { return [S.posteStage === 'Autre poste' ? S.posteStageLibre : S.posteStage, S.dureeStage, S.entrepriseS].filter(Boolean).join(' · ') },
    champs: [
      ...
      { k:'entrepriseS', l:'Nom de l’entreprise', t: 'text' as const, req:true, ph:'Ex : Cabinet Diallo, Clinique Sainte-Marie' },
  ...
  'Freelance': {
    ...
    titre: function (S: Vals) { return [S.prestation === 'Autre poste' ? S.prestationLibre : S.prestation, S.uniteTarif].filter(Boolean).join(' · ') },
    champs: [
      ...
      { k:'refs', l:'Références', req:true,
        when:function (S: Vals) { return /prestataire/.test(S.sensFree || '') },
  ...
  'Emploi maison': {
    ...
    titre: function (S: Vals) { return [S.posteMaison, S.tempsMaison, S.logement].filter(Boolean).join(' · ') },
    champs: [
      ...
      { k:'enfantsMaison', l:'Nombre d’enfants à garder',
        when:function (S: Vals) { return /Nounou|garde d’enfants/i.test(S.posteMaison || '') || (S.tachesMaison || []).indexOf('Garde d’enfants') > -1 },
...
export const EMPLOI: DonneesCat = {
  sous: SOUS,
