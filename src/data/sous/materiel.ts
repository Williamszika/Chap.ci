...
const SOUS = ['Restauration & Maquis', 'Boutique & Commerce', 'Agriculture & Élevage',
...
function heures(): ChampCourt {
  return { k:'heures', l:'Heures de fonctionnement', t: 'num' as const, unite:'h', ph:'Ex : 4500',
    h:function (S: Vals) {
...
function communsPro(): ChampCourt[] {
  return [
    heures(),
...
const SCHEMAS: Record<string, SchemaSous> = {
  ...
  'Restauration & Maquis': {
    ...
    titre: function (S: Vals) { return [S.typeResto, S.marquePro, S.capaciteResto].filter(Boolean).join(' · ') },
    champs: ([
      ...
      { k:'marquePro', l:'Marque', t: 'text' as const, ph:'Ex : Bosch, Nasco, fabrication locale' },
      { k:'capaciteResto', l:'Capacité',
        when:function (S: Vals) { return /froide|Congélateur|Vitrine|Réfrigérateur|Four|Friteuse|glaçons|Pétrin/.test(S.typeResto || '') },
    ...
    ] as ChampCourt[]).concat(communsPro())
  },

  'Boutique & Commerce': {
    couleurs: false, etat: true, livraison: false, prixLabel: 'Prix',
    sansCouleur: 'Montrez le matériel installé si possible : un commerçant se projette mieux.',
    titre: function (S: Vals) { return [S.typeBoutique, S.marquePro].filter(Boolean).join(' · ') },
    champs: ([
      ...
      { k:'marquePro', l:'Marque', t: 'text' as const, ph:'Ex : Casio, Sam4s, fabrication locale' },
      { k:'homologBalance', l:'Vérification métrologique', req:true,
        when:function (S: Vals) { return /Balance/.test(S.typeBoutique || '') },
      ...
      { k:'dimsPro', l:'Dimensions', t: 'text' as const, ph:'Ex : 200 × 90 × 45 cm' }
    ] as ChampCourt[]).concat(communsPro())
  },
  ...
  'Agriculture & Élevage': {
    ...
    titre: function (S: Vals) { return [S.typeAgri, S.marquePro, S.heures ? S.heures + ' h' : ''].filter(Boolean).join(' · ') },
    champs: ([
      ...
      { k:'marquePro', l:'Marque', t: 'text' as const, ph:'Ex : Honda, Kipor, sans marque' },
      { k:'debit', l:'Débit ou capacité', t: 'text' as const, ph:'Ex : 30 m³/h, 500 kg/h',
  ...
  'Industrie & Atelier': {
    ...
    titre: function (S: Vals) { return [S.typeIndus, S.marquePro, S.heures ? S.heures + ' h' : ''].filter(Boolean).join(' · ') },
    champs: ([
      ...
      { k:'marquePro', l:'Marque', t: 'text' as const },
      { k:'puissancePro', l:'Puissance', t: 'text' as const, ph:'Ex : 5,5 kW, 10 kVA, 3 CV' },
  ...
  'Bureau & Informatique': {
    ...
    titre: function (S: Vals) { return [S.typeBureau, S.marquePro].filter(Boolean).join(' · ') },
    champs: ([
      ...
      { k:'marquePro', l:'Marque', t: 'text' as const, ph:'Ex : Canon, Ricoh, Konica' },
      { k:'compteurCopies', l:'Compteur de copies', t: 'num' as const, unite:'copies', ph:'Ex : 120000',
        when:function (S: Vals) { return /Photocopieur|Imprimante|Scanner/.test(S.typeBureau || '') },
        h:function (S: Vals) {
      ...
      { k:'consommables', l:'Consommables', req:true,
        when:function (S: Vals) { return /Photocopieur|Imprimante|Relieuse|Plastifieuse|Vidéoprojecteur/.test(S.typeBureau || '') },
  ...
  'Salon & Esthétique': {
    ...
    titre: function (S: Vals) { return [S.typeSalon, S.marquePro].filter(Boolean).join(' · ') },
    champs: ([
  ...
  'Médical & Paramédical': {
    ...
    titre: function (S: Vals) { return [S.typeMed, S.marquePro].filter(Boolean).join(' · ') },
    champs: ([
...
export const MATERIEL: DonneesCat = {
  sous: SOUS,
