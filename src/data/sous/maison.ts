...
const SOUS = ['Meubles', 'Électroménager', 'Décoration', 'Cuisine', 'Jardin & Bricolage', 'Literie']
...
const TOUTES_COULEURS: Couleur[] = PALETTE_BASE.concat(PALETTE_BOIS)
...
function matiereMeuble(): ChampCourt {
...
function surMesure(quoi: string): ChampCourt[] {
  return [
    ...
    { k:'delai', l:'Délai de fabrication ou de livraison', req:true,
      when:function (S: Vals) { return /Sur commande/.test(S.dispo || '') },
...
function dimensions(): ChampCourt {
  return { k:'dims', l:'Dimensions (L × l × H)', t: 'text' as const, req:true, ph:'Ex : 180 × 85 × 75 cm',
...
function livraisonEtage(): ChampCourt[] {
  return [
    ...
    { k:'etage', l:'Livraison à l’étage', multi:true,
      when:function (S: Vals) { return /Livraison/.test(S.livrDetail || '') },
...
function energie(): ChampCourt[] {
  return [
    ...
    { k:'puissanceW', l:'Puissance', t: 'num' as const, unite:'W', ph:'Ex : 150',
...
function etatMarche(quoi: string): ChampCourt {
...
const SCHEMAS: Record<string, SchemaSous> = {
  ...
  'Meubles': {
    ...
    titre: function (S: Vals) { return [S.typeMeuble, S.matiere, S.dims].filter(Boolean).join(' · ') },
    champs: ([
      ...
      { k:'traite', l:'Bois traité contre les termites', req:true,
        when:function (S: Vals) { return /Bois|Contreplaqué|Rotin/.test(S.matiere || '') },
      ...
      { k:'places', l:'Nombre de places',
        when:function (S: Vals) { return /Canapé|Salon complet|Fauteuil|Table à manger|Ensemble/.test(S.typeMeuble || '') },
        o:['1','2','3','4','5','6','7','8 et plus'] },
      { k:'revetement', l:'Revêtement',
        when:function (S: Vals) { return /Canapé|Fauteuil|Salon|Pouf|Chaise|Lit|Balancelle/.test(S.typeMeuble || '') },
      ...
      ] as ChampCourt[]).concat(surMesure('un meuble fabriqué à la demande'), livraisonEtage(), [
  ...
  'Électroménager': {
    ...
    couleurs: function (S: Vals) { return /Réfrigérateur|Congélateur|Cuisinière|Micro-ondes|Lave-linge|Climatiseur/.test(S.typeElec || '') },
    ...
    titre: function (S: Vals) { return [S.typeElec, S.marqueElec, S.capaciteElec, S.classeEnergie && S.classeEnergie !== 'Non indiquée' ? 'classe ' + S.classeEnergie : ''].filter(Boolean).join(' · ') },
    champs: ([
      ...
      { k:'refElec', l:'Référence', t: 'text' as const, ph:'Ex : RT29K5030S8',
      ...
      { k:'capaciteElec', l:'Capacité',
        when:function (S: Vals) { return /Réfrigérateur|Congélateur|Lave-linge|Sèche-linge|Micro-ondes|Four/.test(S.typeElec || '') },
      ...
      { k:'btu', l:'Puissance de froid', req:true,
        when:function (S: Vals) { return S.typeElec === 'Climatiseur' },
      ...
      { k:'installation', l:'Installation', req:true,
        when:function (S: Vals) { return /Climatiseur|Chauffe-eau|Lave-linge|Cuisinière/.test(S.typeElec || '') },
  ...
  'Décoration': {
    ...
    titre: function (S: Vals) { return [S.typeDeco, S.origineDeco === 'Artisanat ivoirien fait main' ? 'artisanat ivoirien' : '', S.dimsDeco].filter(Boolean).join(' · ') },
    champs: ([
      ...
      { k:'dimsDeco', l:'Dimensions', t: 'text' as const, req:true, ph:'Ex : 120 × 80 cm, hauteur 45 cm',
      ...
      { k:'fixation', l:'Fixation',
        when:function (S: Vals) { return /Tableau|Miroir|Luminaire|Horloge|Tenture|Rideaux/.test(S.typeDeco || '') },
        o:['Prêt à poser, accroches fournies','Accroches à prévoir','Fixation murale nécessaire','Pose comprise'] }
    ] as ChampCourt[]).concat(livraisonEtage())
  },

  /* ================================================================== */
  'Cuisine': {
    etat: true, livraison: false,
    couleurs: function (S: Vals) { return /Vaisselle|Casserole|Ustensile|Textile|Rangement|Service/.test(S.typeCuisine || '') },
    sansCouleur: function (S: Vals) {
    ...
    titre: function (S: Vals) { return [S.typeCuisine, S.matiereCuisine, S.piecesCuisine].filter(Boolean).join(' · ') },
    champs: ([
      ...
      { k:'matiereCuisine', l:'Matière',
        when:function (S: Vals) { return !/gaz|Plaque|Blender|Bouilloire|Cafetière|Friteuse/i.test(S.typeCuisine || '') },
      ...
      { k:'piecesCuisine', l:'Nombre de pièces',
        when:function (S: Vals) { return /Vaisselle|Casseroles|Ustensiles|Batterie|Rangement|Textile/.test(S.typeCuisine || '') },
      ...
      { k:'tailleBouteille', l:'Taille de la bouteille', req:true,
        when:function (S: Vals) { return S.typeCuisine === 'Bouteille de gaz' },
      ...
      { k:'feux', l:'Nombre de feux',
        when:function (S: Vals) { return /Réchaud|Plaque/.test(S.typeCuisine || '') },
  ...
  'Jardin & Bricolage': {
    etat: true, livraison: false,
    couleurs: function (S: Vals) { return /Mobilier de jardin|Parasol|Pot|Arrosage/.test(S.typeJardin || '') },
    sansCouleur: function (S: Vals) {
    ...
    titre: function (S: Vals) { return [S.typeJardin, S.marqueOutil, S.puissanceOutil ? S.puissanceOutil + ' W' : ''].filter(Boolean).join(' · ') },
    champs: ([
      ...
      { k:'marqueOutil', l:'Marque', t: 'text' as const, ph:'Ex : Bosch, Makita, Kipor, sans marque' },
      etatMarche('un outil'),
      { k:'alimOutil', l:'Alimentation', req:true,
        when:function (S: Vals) { return !/Mobilier|Parasol|Pot|Arrosage|Échelle|Outils à main|Peinture/.test(S.typeJardin || '') },
        o:['Secteur 220 V','Batterie rechargeable','Essence','Diesel','Manuel'] },
      { k:'puissanceOutil', l:'Puissance', t: 'num' as const, unite:'W', ph:'Ex : 800',
        when:function (S: Vals) { return /Perceuse|Meuleuse|Scie|Ponceuse|Poste|Compresseur|Pompe|Tondeuse/.test(S.typeJardin || '') } },
      { k:'puissanceKva', l:'Puissance du groupe', req:true,
        when:function (S: Vals) { return S.typeJardin === 'Groupe électrogène' },
      ...
      { k:'heuresOutil', l:'Heures de fonctionnement', t: 'num' as const, unite:'h', ph:'Ex : 400',
        when:function (S: Vals) { return /Groupe électrogène|Compresseur|Pompe/.test(S.typeJardin || '') },
      ...
      { k:'usagePro', l:'Usage précédent',
        when:function (S: Vals) { return !/Mobilier|Parasol|Pot|Peinture/.test(S.typeJardin || '') },
  ...
  'Literie': {
    etat: true, livraison: false,
    couleurs: function (S: Vals) { return !/Matelas|Sommier/.test(S.typeLiterie || '') },
    sansCouleur: 'Un matelas est blanc. Photographiez plutôt l’étiquette, les coins, et la tranche pour montrer l’épaisseur.',
    aideCouleurs: 'Cochez chaque couleur disponible. Ouvrez-en une pour lui donner ses photos, son prix et un détail court.',
    titre: function (S: Vals) { return [S.typeLiterie, S.taille, S.epaisseur].filter(Boolean).join(' · ') },
    champs: ([
      ...
      { k:'garnissage', l:'Garnissage', req:true,
        when:function (S: Vals) { return /Matelas|Ensemble/.test(S.typeLiterie || '') },
      ...
      { k:'densite', l:'Densité de la mousse',
        when:function (S: Vals) { return /Mousse/.test(S.garnissage || '') },
      ...
      { k:'epaisseur', l:'Épaisseur',
        when:function (S: Vals) { return /Matelas|Ensemble|Protège/.test(S.typeLiterie || '') },
        o:['10 cm','15 cm','20 cm','25 cm','30 cm et plus'] },
      { k:'matiereLit', l:'Matière',
        when:function (S: Vals) { return /Drap|Couette|Couverture|Oreiller|Traversin|Tête de lit|Moustiquaire/.test(S.typeLiterie || '') },
