...
var SOUS = ['Produits vivriers', 'Fruits & Légumes', 'Céréales & Tubercules', 'Épices & Condiments',
...
function quantite(exemple) {
  return [
    ...
    { k:'poids', l:'Poids ou volume approximatif', t:'text', req:true, ph:exemple,
    ...
    { k:'gros', l:'Vente en gros possible', t:'toggle',
...
var SCHEMAS = {
  ...
  'Produits vivriers': {
    ...
    titre: function (S) { return [S.produitV, S.poids, S.unite].filter(Boolean).join(' · ') },
    champs: [
    ...
    ].concat(quantite('Ex : environ 5 kg la cuvette'))
  },

  'Fruits & Légumes': {
    couleurs: false, etat: false, livraison: true, prixLabel: 'Prix',
    sansCouleur: 'Une photo prise à la lumière du jour, sans flash : c’est ce qui montre la fraîcheur.',
    titre: function (S) { return [S.produitF, S.poids, S.unite].filter(Boolean).join(' · ') },
    champs: [
      ...
      froid()
    ].concat(quantite('Ex : cageot de 20 kg'))
  },

  'Céréales & Tubercules': {
    couleurs: false, etat: false, livraison: true, prixLabel: 'Prix',
    sansCouleur: 'Photographiez le grain de près, et le sac ouvert : c’est ce qu’un acheteur en gros regarde.',
    titre: function (S) { return [S.produitC, S.poids, S.unite].filter(Boolean).join(' · ') },
    champs: [
  ...
  'Épices & Condiments': {
    ...
    ].concat(quantite('Ex : sachet de 250 g'))
  },

  'Produits du terroir': {
    couleurs: false, etat: false, livraison: true, prixLabel: 'Prix',
    sansCouleur: 'Montrez le produit et son étiquette. Sur du terroir, l’origine fait le prix.',
    titre: function (S) { return [S.produitT, S.regionT, S.poids].filter(Boolean).join(' · ') },
    champs: [
    ...
    ].concat(quantite('Ex : pot de 500 g'))
  },

  'Boissons': {
    couleurs: false, etat: false, livraison: true, prixLabel: 'Prix',
    sansCouleur: 'Photographiez l’étiquette lisiblement, avec la date.',
    titre: function (S) { return [S.produitB, S.contenanceB, S.unite].filter(Boolean).join(' · ') },
    champs: [
      ...
      froid()
    ].concat(quantite('Ex : casier de 12 bouteilles'))
  },
  ...
  'Plats préparés': {
    ...
    titre: function (S) { return [S.plat, S.portions].filter(Boolean).join(' · ') },
    champs: [
  ...
  'Cacao & Café': {
    ...
    titre: function (S) { return [S.produitCC, S.campagne, S.poids].filter(Boolean).join(' · ') },
    champs: [
      ...
      { k:'fermentation', l:'Fermentation',
        when:function (S) { return /cacao|Cabosses/i.test(S.produitCC || '') },
    ...
    ].concat(quantite('Ex : sacs de 65 kg'))
  },

  'Semences & Intrants': {
    couleurs: false, etat: false, livraison: true, prixLabel: 'Prix',
    sansCouleur: 'Photographiez l’étiquette du sac : numéro de lot, date, et composition.',
    titre: function (S) { return [S.typeIntrant, S.cultureCible, S.poids].filter(Boolean).join(' · ') },
    champs: [
      ...
      { k:'cultureCible', l:'Pour quelle culture', multi:true,
        when:function (S) { return !/Aliment|vétérinaire/.test(S.typeIntrant || '') },
      ...
      { k:'homologation', l:'Homologation', req:true,
        when:function (S) { return /Pesticide|Herbicide|Fongicide|vétérinaire|Engrais/.test(S.typeIntrant || '') },
      ...
      { k:'varieteS', l:'Variété', t:'text',
        when:function (S) { return /Semences|Plants/.test(S.typeIntrant || '') }, ph:'Ex : maïs EV8728, hévéa GT1' },
      { k:'germination', l:'Taux de germination',
        when:function (S) { return /Semences/.test(S.typeIntrant || '') },
        o:['Plus de 90 %','80 à 90 %','70 à 80 %','Moins de 70 %','Non testé'] },
      peremption('un intrant')
    ].concat(quantite('Ex : sac de 50 kg'))
  },
  ...
  'Poisson & Produits de mer': {
    ...
    titre: function (S) { return [S.produitP, S.etatP, S.poids].filter(Boolean).join(' · ') },
    champs: [
    ...
    ].concat(quantite('Ex : caisse de 20 kg'))
  }
}
