...
var SOUS = ['BTP & Rénovation', 'Cours & Formation', 'Événementiel', 'Transport & Déménagement', 'Informatique & Digital', 'Couture & Artisanat']

var PALETTE_BASE = []
var TOUTES_COULEURS = []

var EXPERIENCE = ['Débutant','1 à 2 ans','3 à 5 ans','5 à 10 ans','Plus de 10 ans']
var ZONES = ['Ma commune uniquement','Abidjan Sud (Marcory, Koumassi, Treichville, Port-Bouët)','Abidjan Nord (Abobo, Adjamé, Attécoubé)','Abidjan Est (Cocody, Bingerville)','Abidjan Ouest (Yopougon, Songon)','Tout Abidjan','Toute la Côte d’Ivoire','À distance / en ligne']
...
var SCHEMAS = {
  ...
  'BTP & Rénovation': {
    ...
    titre: function (S) { return [(S.metier || []).slice(0,2).join(', '), S.uniteBtp].filter(Boolean).join(' · ') },
    champs: [
  ...
  'Cours & Formation': {
    ...
    titre: function (S) { return [(S.matiere || []).slice(0,2).join(', '), S.niveauEleve, S.uniteCours].filter(Boolean).join(' · ') },
    champs: [
    ...
    ].concat(communs('une formation'))
  },

  'Événementiel': {
    couleurs: false, etat: false, livraison: false, prixLabel: 'Tarif à partir de',
    sansCouleur: 'Montrez vos prestations passées : sur l’événementiel, tout se décide sur les photos.',
    titre: function (S) { return [(S.prestaEvt || []).slice(0,2).join(', '), S.typeEvt].filter(Boolean).join(' · ') },
    champs: [
  ...
  'Transport & Déménagement': {
    ...
    titre: function (S) { return [(S.prestaT || []).slice(0,2).join(', '), S.vehicule].filter(Boolean).join(' · ') },
    champs: [
      ...
      { k:'licence', l:'Autorisation de transport', req:true,
        when:function (S) { return (S.prestaT || []).indexOf('Transport de personnes') > -1 },
    ...
    ].concat(communs('un transport'))
  },

  'Informatique & Digital': {
    couleurs: false, etat: false, livraison: false, prixLabel: 'Tarif',
    sansCouleur: 'Montrez vos réalisations : un portfolio vend mieux que n’importe quelle description.',
    titre: function (S) { return [(S.prestaI || []).slice(0,2).join(', '), S.uniteI].filter(Boolean).join(' · ') },
    champs: [
    ...
    ].concat(communs('un projet'))
  },

  'Couture & Artisanat': {
    couleurs: false, etat: false, livraison: false, prixLabel: 'Tarif à partir de',
    sansCouleur: 'Montrez vos modèles finis, portés si possible. En couture, tout se décide là.',
    titre: function (S) { return [(S.prestaC || []).slice(0,2).join(', '), S.delaiC].filter(Boolean).join(' · ') },
    champs: [
