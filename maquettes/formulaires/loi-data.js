...
var SOUS = ['Sport & Fitness', 'Vélos & Trottinettes', 'Instruments de musique', 'Livres & BD', 'Jeux de société & Puzzles', 'Collections']
...
var SCHEMAS = {
  ...
  'Sport & Fitness': {
    ...
    titre: function (S) { return [S.typeSport, S.marqueL, S.tailleSport].filter(Boolean).join(' · ') },
    champs: [
      ...
      { k:'arme', l:'S’agit-il d’une arme ?', req:true,
        when:function (S) { return /Chasse|Pêche|Arts martiaux|Boxe/.test(S.disciplineS || '') || /chasse|pêche/i.test(S.typeSport || '') },
      ...
      { k:'marqueL', l:'Marque', t:'text', ph:'Ex : Adidas, Decathlon, sans marque' },
      { k:'tailleSport', l:'Taille',
        when:function (S) { return /Tenue|Chaussures|Gants|Protection|Sac/.test(S.typeSport || '') },
        o:['XS','S','M','L','XL','2XL','3XL','Taille unique','Enfant','Pointure 36-40','Pointure 41-45'] },
      { k:'poidsSport', l:'Poids ou charge', t:'text',
        when:function (S) { return /Haltères|poids|Appareil|Banc/.test(S.typeSport || '') }, ph:'Ex : 2 × 10 kg' },
  ...
  'Vélos & Trottinettes': {
    ...
    titre: function (S) { return [S.typeVelo, S.marqueL, S.tailleVelo].filter(Boolean).join(' · ') },
    champs: [
      ...
      { k:'marqueL', l:'Marque', t:'text', ph:'Ex : Btwin, Rockrider, sans marque' },
      ...
      { k:'batterieVelo', l:'Batterie', req:true,
        when:function (S) { return /électrique/.test(S.typeVelo || '') },
      ...
      { k:'autonomieVelo', l:'Autonomie annoncée', t:'num', unite:'km',
        when:function (S) { return /électrique/.test(S.typeVelo || '') } },
  ...
  'Instruments de musique': {
    ...
    titre: function (S) { return [S.instrument, S.marqueL, S.tonalite].filter(Boolean).join(' · ') },
    champs: [
      ...
      { k:'marqueL', l:'Marque', t:'text', ph:'Ex : Yamaha, Fender, fabrication artisanale' },
      { k:'tonalite', l:'Tonalité / taille',
        when:function (S) { return /Cordes|Vents|Claviers/.test(S.familleInstru || '') },
  ...
  'Livres & BD': {
    ...
    titre: function (S) { return [S.genreLivre, S.nbLivres, S.langueLivre].filter(Boolean).join(' · ') },
    champs: [
      ...
      { k:'niveauLivre', l:'Niveau',
        when:function (S) { return /Scolaire|Parascolaire|Universitaire/.test(S.genreLivre || '') },
      ...
      { k:'anneeEdition', l:'Année d’édition', t:'text', ph:'Ex : 2023',
  ...
  'Jeux de société & Puzzles': {
    ...
    titre: function (S) { return [S.typeJeuS, S.ageJeu, S.joueurs].filter(Boolean).join(' · ') },
    champs: [
      ...
      { k:'piecesPuzzle', l:'Nombre de pièces',
        when:function (S) { return /Puzzle/.test(S.typeJeuS || '') },
  ...
  'Collections': {
    ...
    titre: function (S) { return [S.typeCollec, S.epoque, S.nbCollec].filter(Boolean).join(' · ') },
    champs: [
